        ...
        <List sx={{ width: '100%', bgcolor: '#fff', height: 80 }} disablePadding key={myProfile['user_profiles'][0][0]}>
          <ListItem alignItems="flex-start" disablePadding>
            ...
            <ListItemText
              ...
              secondary={
                new Date(
                  Date.now() + (new Date(myProfile['user_profiles'][0][5]).getTimezoneOffset() + 9 * 60) * 60 * 1000
                ).getHours() +
                ':' +
                new Date(
                  Date.now() + (new Date(myProfile['user_profiles'][0][5]).getTimezoneOffset() + 9 * 60) * 60 * 1000
            ...
            {himaFriends.map((friend: OthersNotification) => (
              <ImageListItem key={friend.friend} style={{ alignItems: 'center', justifyContent: 'space-even' }}>
                ...
                <Typography style={{ color: '#808080', fontSize: 12 }}>
                  {new Date(Date.now() + (new Date(friend.login_at).getTimezoneOffset() + 9 * 60) * 60 * 1000).getHours() +
                    ':' +
                    new Date(Date.now() + (new Date(friend.login_at).getTimezoneOffset() + 9 * 60) * 60 * 1000).getMinutes() +
                    ' に暇になった'}{' '}
                </Typography>
              </ImageListItem>
            ))}
